...
let playerOne = "You beat the computer!";
let playerTwo = "The computer has won!";
let noOfturns = 0;
//Using new Image object to preload images for the board. Images are being hosted off my dropbox
let X = new Image();
let O = new Image();
O.src = "/Images/O image.png";
X.src = "/Images/X image.png";
    ...
    for (let i = 1; i < board.length; i++) {
      for (j = 1; j < board.length; j++) {
        if (id == board[i][j]) {
          if (
            turn == "X" &&
            document.getElementById(board[i][j]).innerHTML == ""
          ) {
            document.getElementById(board[i][j]).innerHTML =
              "<img src =" + X.src + ">";
...
function checkWinner(name) {
  ...
  for (i = 1; i < board.length; i++) {
    ...
    if (
      document.getElementById(board[i][1]).innerHTML ==
        document.getElementById(board[i][2]).innerHTML &&
      document.getElementById(board[i][2]).innerHTML ==
    ...
    // checks the cols and compares if the images are the same
      ...
      document.getElementById(board[1][i]).innerHTML ==
        document.getElementById(board[2][i]).innerHTML &&
      document.getElementById(board[2][i]).innerHTML ==
    ...
    // checks the left diagonal for a win
      ...
      document.getElementById(board[1][1]).innerHTML ==
        document.getElementById(board[2][2]).innerHTML &&
      document.getElementById(board[2][2]).innerHTML ==
    ...
    // checks the right diagonal for a win
      ...
      document.getElementById(board[1][3]).innerHTML ==
        document.getElementById(board[2][2]).innerHTML &&
      document.getElementById(board[2][2]).innerHTML ==
  ...
  if (winner) {
    document.getElementById("playerWon").innerHTML = name;
    if (name == playerOne) {
      document.getElementById("playerWon").style.color = "#219653";
    } else {
      document.getElementById("playerWon").style.color = "#EB5757";
    }
    //   winner = false;
  }
  if (!winner && noOfturns == 9) {
    document.getElementById("playerWon").innerHTML = "DRAW";
    document.getElementById("playerWon").style.color = "#BDBDBD";
  }
}
